...
class RctCollapsibleCard extends Component {
    ...
    onReload() {
        ...
        if(this.props.reloadebleFunction)
        {
            this.props.reloadebleFunction()
        }
        let self = this;
        setTimeout(() => {
    ...
    render() {
        ...
        const { children, collapsible, closeable, reloadable,reloadebleFunction, heading, fullBlock, colClasses, customClasses, headingCustomClasses, contentCustomClasses, badge } = this.props;
        return (
                        ...
                        <div className={`rct-block-title ${headingCustomClasses ? headingCustomClasses : ''}`}>
                            ...
                            {(collapsible || reloadable || closeable) &&
                                <div className="contextual-link">
                                    ...
                                    {reloadable && 
                                        <a href="javascript:void(0)" onClick={() => this.onReload()}><i className="ti-reload"></i></a>
                                     }
