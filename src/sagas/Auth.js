 ...
 * Sigin User With Email and Password Request
 */
const signInUserWithEmailPasswordRequestWithDB = async (email, password) =>
    await api.post('user/login',{email, password , 'role':2, })
        .then(authUser => authUser)
        .catch(error => error);
/**
...
const createUserWithEmailPasswordRequestWithDB = async (email, password, full_name ,lang) =>
     await api.post('user/register',{email, password, full_name,lang})
        .then(authUser => authUser)
        .catch(error => error);
...
function* createUserWithEmailPassword({ payload }) {
    ...
    try {
        ...
        if (signUpUser.message) {
          ...
          //  NotificationManager.success(signUpUser.success_message);
 ...
 */
export function* signinUserInDB() {
    yield takeEvery(LOGIN_USER_DB, signInUserWithEmailPasswordWithDB);
    
}



function* signInUserWithEmailPasswordWithDB({ payload }) {
    const { email, password } = payload.user;
    const { history } = payload;
    try {
        const signUpUser = yield call(signInUserWithEmailPasswordRequestWithDB, email, password);
        
        if (signUpUser.data.message) {
            yield put(signinUserFailure(signUpUser.data.message));
        } else {

            localStorage.setItem('user_id', JSON.stringify(signUpUser.data.data));
            yield put(signinUserSuccess(JSON.stringify(signUpUser.data.data)));
            history.push('/')
        }
    } catch (error) {
        yield put(signinUserFailure(error));
 ...
 * Login User with database
 */
export function* signupUserWithDB() {
    yield takeEvery(SIGNUP_USER_DB, createUserWithEmailPasswordWithDB);
    
}



function* createUserWithEmailPasswordWithDB({ payload }) {
    const { name, email, password ,lang} = payload.user;
    const { history } = payload
    try {
        const signUpUser = yield call(createUserWithEmailPasswordRequestWithDB, email, password, name ,lang);
        
       
        if (signUpUser.data.message) {
            yield put(signUpUserInFirebaseFailure(signUpUser.data.message));
        } else {
           // localStorage.setItem('user_id', JSON.stringify(signUpUser.data.data.id));
            //console.log('signUpUser',signUpUser)
          //  NotificationManager.success(signUpUser.data.success_message);
            yield put(signUpUserInFirebaseSuccess(JSON.stringify(signUpUser.data.data)));
            setTimeout(() => {
               // history.push('/')
              }, 500); 
        }
