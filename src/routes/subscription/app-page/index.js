...
import {
	...
	hubCheckPaths, getOsNamesByIds, user_id, convertDateToTimeStamp, get_plan_name_by_id, dateMonthsDiff,h_not_in_array, inArray
...
class AppPage extends Component {
	...
	onDelete(data ,is_child_subscription ,broswser) {
		console.log('onDelete data',data)
		this.setState({ selectedUser: broswser });
	...
	cancelTransactionPermanently(subscription_id=null,is_transaction_cancel) {
		const { selectedUser } = this.state;
		console.log('selectedUser',selectedUser)
		
		if(selectedUser.is_child_subscription==0 && is_transaction_cancel == 2  ){
			this.refs.deleteConfirmationDialog.close();     
			setTimeout(() => {
				this.refs.deleteConfirmationDialogTransactionAlsoOrNot.open();
			}, 500);
			return
		}

		console.log('is_transaction_cancel',is_transaction_cancel)
		...
		if(subscription_id){
			...
			fun_subscription_id =selectedUser.serial_data.subscription_id;
		}
		let users = this.state.currentpagedataSub;
		this.refs.deleteConfirmationDialogTransactionAlsoOrNot.close();
		this.refs.deleteConfirmationDialog.close();
		...
		api.post('cancle-subscription', {
			...
			'is_transaction_cancel': is_transaction_cancel,
		}).then((response) => {
	...
	openUpgradePlanModal(key, sub_transaction, browser_language ,not_expired_plans=[],serial_data , user_has_active_device,user_has_active_device_data) {
	...
	installAppPopup(key,not_expired_plans=[], user_has_active_device = 0, user_has_active_device_data=[] , user_old_installed_data = []) {
		console.log('not_expired_plans',not_expired_plans)


		console.log('aksaa',user_has_active_device_data)
		this.setState({
			openViewUserDialog: true,
			popupBrowserId: key,
			upgradepopupdetal: [],
			...
			user_has_active_device:user_has_active_device,
			user_has_active_device_data:user_has_active_device_data,
			user_old_installed_data:user_old_installed_data
		});
	}
            ...
            />
							...
							<div class="product type-3 white-bg z-depth-1 text-center rct-block">
								...
								{broswser.is_install != 1 ?
											<div>
												...
												<div class="price form-group">
													<button class="btn btn-primary" style={{ width: '90%' }} onClick={() => this.installAppPopup(key,[],broswser.user_has_active_device, broswser.user_has_active_device_data , broswser.user_old_installed_data  )}  ><IntlMessages id="widgets.Install" /></button>
												</div>
											</div>
											:
											broswser.serial_data.is_expired == 1 ?
												<div>
													...
													<div class="price form-group">
														<button class="btn btn-primary" style={{ width: '90%' }} onClick={() => this.installAppPopup(key,broswser.not_expired_plans,broswser.user_has_active_device, broswser.user_has_active_device_data , broswser.user_old_installed_data)}  ><IntlMessages id="widgets.Install" /></button>
													</div>
												</div>
											:
											<div>
											...
											{broswser.is_child_subscription == 0 &&
												<div className="form-group">
													...
													}}> <span><button class="btn btn-white" style={{ backgroundColor: "#5D92F4", color: "white", width: '100%' }}  ><IntlMessages id="welcome.contentmanager" /></button></span></Link>

												</div>
										 	} 

												<div className="form-group">
													<span><button id={"modifySubscription"+broswser.id} class={"btn btn-white modifySubscription"} onClick={() => this.openUpgradePlanModal(key, broswser.transaction_data, broswser.language , broswser.not_expired_plans ,broswser.serial_data,broswser.user_has_active_device , broswser.user_has_active_device_data)} style={{ backgroundColor: "#5D92F4", color: "white", width: '90%' }}><IntlMessages id="widgets.ModifySubscription" /></button></span>
												</div>

												<div class="price form-group" >

													{/* {broswser.transaction_data.is_subscription_canceled == 0 &&                                   */}
													<a href="javascript:void(0)" style={{ width: '90%' }} onClick={() => this.onDelete(broswser.serial_data , broswser.is_child_subscription , broswser)}><button class="btn btn-danger install-btn" style={{ width: '100%' }} ><IntlMessages id="widgets.Uninstall" /></button></a>
													{/* } */}
												</div>
												{(window.IsChameleon && broswser.transaction_data.is_launched == 1 ) &&
													<div  class="price form-group" 
													 ...
													 >
														 ...
														 <button class="btn btn-success " style={{ width: '100%' }} ><IntlMessages id="widgets.LaunchTheApp" /></button></a> 
														 {localStorage.getItem("f_os") ==2 ?
														 (broswser.user_xml_os ?
															<Label>{ broswser.user_xml_os.setting_exit_key ? broswser.user_xml_os.setting_exit_key : 'Command + L'} <IntlMessages id="widgets.toQuit"/></Label>
															:
															//window.IsChameleon &&
															<Label>Command + L <IntlMessages id="widgets.toQuit"/></Label>
														 ) 
														 :
														(broswser.user_xml_os ?
												...
												{/* {console.log('kapiltest',broswser.user_xml_os.os_type)} */}
				...
				/> */}
					...
					<DeleteConfirmationDialog
						...
						title={this.state.selectedUser&& "Are you sure you want to uninstall "+this.state.selectedUser.name}
						// message={<IntlMessages id="wedgit.sureToCancelSubsNote" />}
					//	onCancel={() => this.cancelTransactionPermanently(null,false)}
						onConfirm={() => this.cancelTransactionPermanently(null,2)}
						btnCancel={<IntlMessages id="button.no" />}
						btnYes={<IntlMessages id="button.yes" />}
					/>
				
					<DeleteConfirmationDialog
						ref="deleteConfirmationDialogTransactionAlsoOrNot"
						// title={"Are you sure you want to cancel '"+this.state.upgradepopupdetal.transactionss.plan_name+"' subscription ?"}
						title={this.state.selectedUser&& "Would you like to unsubscribe from "+this.state.selectedUser.name}
						message={<IntlMessages id="wedgit.sureToCancelSubsNote" />}
						onCancel={() => this.cancelTransactionPermanently(null,0)}
						onConfirm={() => this.cancelTransactionPermanently(null,1)}
						btnExtra={<IntlMessages id="button.cancel" />}
						btnCancel={<IntlMessages id="button.no" />}
						btnYes={<IntlMessages id="button.yes" />}
					/>
						...
						this.state.upgradepopupdetal.transactionss.plan_name == get_plan_name_by_id(AppConfig.plans_array, 1)?
							...
							:
								...
								:
									...
									:
										<Button color={"warning"}  onClick={()=> this.installAppAgain(this.state.popupBrowserId,2,this.state.upgradepopupdetal.serial_data)} 
										className="btn-block btn-lg" style={{ width: "113%"}}  >
					...
					:
						h_not_in_array(get_plan_name_by_id(AppConfig.plans_array, 1) ,this.state.not_expired_plans)?
							 inArray(1 , this.state.user_has_active_device)  && this.state.user_has_active_device_data[1].plan_name == get_plan_name_by_id(AppConfig.plans_array, 1) ?
								...
								this.state.user_old_installed_data.status == 0 &&  this.state.user_old_installed_data.is_expired == 1  ?
						...
						inArray(1 , this.state.user_has_active_device)   && this.state.user_has_active_device_data[1].plan_name == get_plan_name_by_id(AppConfig.plans_array, 1) ?
							...
							:
								...
								this.state.user_old_installed_data.status == 0 &&  this.state.user_old_installed_data.is_expired == 1 && this.state.user_old_installed_data.level_type == 1 ?
									<Button color={"warning"}  onClick={()=> this.installApp(this.state.popupBrowserId,1)} 
										className="btn-block btn-lg" style={{ width: "113%"}}  >
										<small><IntlMessages id="widgets.SubscribeTo"/> {get_plan_name_by_id(AppConfig.plans_array, 1)} <IntlMessages id="widgets.for"/> ${this.state.silverPlanYM}/<span class="month">{this.state.planDuration}</span></small>
									</Button>
					...
					{this.state.upgradepopupdetal && this.state.upgradepopupdetal.subs_ids ?
						...
						<Button color={"zac"} className="btn-block btn-lg" style={{ width: "113%"}} >
							...
							<Button color={"zac"}  onClick={()=> this.installAppWhenhasDevice(this.state.user_has_active_device_data[3].code)} 
								className="btn-block btn-lg" style={{ width: "113%"}}  >
								<small><IntlMessages id="widgets.quickActivate" />{console.log('here',this.state.user_has_active_device_data)}</small>
							</Button>
				...
				//	title="To Exit Zac Browser you need to click on CTRL+L Please type CTRL+L on your keyboard to confirm."
					...
					confirmBtnText={
						...
						"Press "+(
							(
								this.state.browserKeyDetail.user_xml_os.setting_exit_key 
							) ?
							 	this.state.browserKeyDetail.user_xml_os.setting_exit_key
								  :
								 'Command + L'
					...
					{localStorage.getItem("f_os") ==2 ?
						...
						<IntlMessages id="widgets.ToExit" /> {this.state.browserKeyDetail.name} <IntlMessages id="widgets.infoExitBrowser" /> {(this.state.browserKeyDetail.user_xml_os.setting_exit_key ) ? this.state.browserKeyDetail.user_xml_os.setting_exit_key : ' Command + L'}</h2>
