	...
	applyPromocodeButton(){
		...
		api.post('promocode_apply_api', {
			...
			if(status == 1){   
				...
				$("#TrialAmount").val(response.data.discount);
				$('.showAmountData').show();  
				$("#TrialFrequency").show();
				$("#TrialAmount").show();
				$("#TrialPeriod").show();         
				...
				<div className="col-lg-8">
					...
					<div className="contact-box addressbox"  style={have_address ? {display:'block'} : {display:'none'}}>
						<form action={PAYPAL_URL} className="p-0" id="payment" method="post">
							...
							<input type="hidden" name="a1" id="TrialAmount" value="7.5"/>
							<input type="hidden" name="p1" id="TrialPeriod" value="1" />
							
							
							{plan_type == 2 ?
							<div>
								<input type="hidden" name="p3" id="paypalValid" value="1"/>
								<input type="hidden" name="t1" id="TrialFrequency"  value="Y"/>
								</div>
							:
							<div>
								<input type="hidden" name="p3" id="paypalValid" value="1"/>
								<input type="hidden" name="t1" id="TrialFrequency" value="M"/>
								</div>
							}
