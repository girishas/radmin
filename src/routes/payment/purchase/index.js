...
// api
import api from 'Api';
import { timeAgo, getTheDate ,checkRoleAuth , get_plan_id_by_name ,get_plan_name_by_id, print_tr_id,user_id} from "Helpers/helpers";
   ...
   render() {
      ...
      return (
         <div className="user-management">
             <IntlMessages id='components.payment' defaultMessage='Chameleon | Subscriptions'>
               {(title) => (
                 <Helmet>
            ...
            <PageTitleBar
               title={<IntlMessages id="components.payment" />}
            ...
            <RctCollapsibleCard fullBlock>
               <div className="table-responsive">
                  <div className="d-flex justify-content-between py-20 px-10 border-bottom">
                     <div>
                      <p onClick={() => this.onReloads()} className="btn-outline-default mr-10">Transactions</p>
                     </div>
                     <div>
                     </div>
                  </div>
                     ...
                     {currentpagedataSub && currentpagedataSub.map((transaction, key)=> (
                        <tr key={key}>
                          <td>{transaction.child_name}</td>
                           <td>{transaction.type == 1 ? transaction.plan_name:""}</td>
							<td>{transaction.type == 1 ? "Purchase":"Donation"}</td>
                         
                           <td>${transaction.amount}</td>
                        
                           <td>{transaction.type == 1 ? print_tr_id(transaction.amount , transaction.payment_type ,transaction.stripe_id , transaction.paypal_tx ):""}</td>
                     ...
                     { (get_plan_id_by_name(transaction.plan_name) == 2) &&
                      <div>
                            ...
                            <td className="list-action">
                                 ...
                                 { /* transaction.is_subscription_canceled == 0 && transaction.type == 1 &&
                     ...
                     <div>
                        <div className="clearfix d-flex">
                              <div className="media-body">
                                 ...
                                 <p><IntlMessages id="components.CreatedAt" />: {timeAgo(selectedTransaction.created_at)}</p>
                              </div>
                        </div>
                     </div>
                  }
