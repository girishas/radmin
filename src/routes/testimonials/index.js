   ...
   render() {
     ...
     if(!users)
      ...
      let filteredResult;
		...
		<IntlMessages id='sidebar.testimonials' defaultMessage='Chameleon | Testimonials'>
		{(title) => (
			<Helmet>
			<title>Chameleon | {title}</title>
			<meta name="description" content="Chameleon | Testimonials" />
			</Helmet>
		)}
		</IntlMessages>
		<PageTitleBar
		title={<IntlMessages id="sidebar.testimonials" />}
