  ...
  constructor() {
   ...
   addNewUser() {
     ...
     const { name, content } = this.state.addNewUserDetail;
      if (name !== '' && content !== '') {
         ...
         api.post('addimage',newUser).then((response) => {
            ...
            data.checked = false;
             ...
             currentCountries.unshift(data);
                setTimeout(() => {
                  ...
                  NotificationManager.success('Image Created!');
               }, 5000);       
         })
         .catch(error => {
            // error hanlding
   ...
   onSort(column){
      ...
      const sortedData = this.state.currentCountries.sort((a, b) => {
        if (column === 'page_title' || column === 'image') {
  ...
  setArrow = (column) => {
         ...
         let newList = null;
          ...
          if (query !== "") {
            ...
            newList = currentList.filter(item => {
                return item.slug.toLowerCase().includes(query.toLowerCase()) || item.page_title.toLowerCase().includes(query.toLowerCase())
              });
          } else {
    ...
    */
    ...
    updateUser() {
      ...
      let self = this;
      api.post('updateimage', editUser).then((response) => {
   ...
   render() {
     ...
     if(!users)
      ...
      let filteredResult;
		...
		<IntlMessages id='sidebar.image' defaultMessage='Chameleon | Image'>
		{(title) => (
			<Helmet>
			<title>Chameleon | {title}</title>
			<meta name="description" content="Chameleon | Image" />
			</Helmet>
		)}
		</IntlMessages>
		<PageTitleBar
		title={<IntlMessages id="sidebar.image" />}
		...
		<a href="javascript:void(0)" onClick={() => this.opnAddNewUserModal()} color="primary" className="caret btn-sm mr-10"><IntlMessages id="sidebar.addnewimage" /> <i className="zmdi zmdi-plus"></i></a> 
		</div>
		</div>
...
{/*    <th onClick={e => this.onSort('slug')}>Slug <i className={this.setArrow('slug')}></i></th>
*/}    <th onClick={e => this.onSort('image')}>{<IntlMessages id="sidebar.image"/>} <i className={this.setArrow('image')}></i></th>
    <th onClick={e => this.onSort('page_title')}>Location <i className={this.setArrow('page_title')}></i></th>
    <th onClick={e => this.onSort('status')}>{<IntlMessages id="widgets.status"/>} <i className={this.setArrow('status')}></i></th>
		...
		{filteredResult && filteredResult.map((user, key) => (
			...
			<td><div className="media">
				{user.image !== '' && user.image !== null && user.image !== 'undefined' ?
				   ...
				   : <Avatar className="mr-15">{user.slug.charAt(0)}</Avatar>
				}
			 </div>
			</td>
      <td>{user.page_title}</td>
		...
		{editUser === null ? <IntlMessages id="sidebar.addnewimage" /> : <IntlMessages id="sidebar.updateimage" /> }
	</ModalHeader>
		<ModalBody>
		{editUser === null ?
			<AddNewUserForm
                        ...
                        <div className="clearfix d-flex">
                           <div className="media pull-left">
                            {selectedUser.image !== '' && selectedUser.image !== null ?
                                       ...
                                       : <Avatar className="mr-15">{selectedUser.slug.charAt(0)}</Avatar>
                                    }
                              <div className="media-body">
                                 <p>Location: <span className="fw-bold">{selectedUser.page_title}</span></p>
