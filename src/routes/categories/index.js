   ...
   addNewUser() {
     ...
     const { name } = this.state.addNewUserDetail;
      ...
      if (name !== '' ) {
         ...
         api.post('addcategory',newUser ).then((response) => {
  ...
  setArrow = (column) => {
    ...
    */
    ...
    updateUser() {
      ...
      let self = this;
      api.post('updatecategory', editUser).then((response) => {
        const data =  response.data;
     ...
     if(search){
      ...
      return (
         <div className="user-management">
             <IntlMessages id='widgets.category' defaultMessage='React | Category Manager'>
               {(title) => (
                 <Helmet>
            ...
            <PageTitleBar
               title={<IntlMessages id="widgets.category" />}
                  ...
                  <table className="table table-middle table-hover mb-0">
                     <thead>
                        <tr>
                           ...
                           <th onClick={e => this.onSort('type')}>Type <i className={this.setArrow('type')}></i></th>
                     ...
                     <tbody>
                        {filteredResult && filteredResult.map((user, key) => (
                           <tr key={key}>
                              ...
                              <td>{user.type == 1 ?
                                 <span className={`badge badge-success badge-pill`}>Blog</span> : <span className={`badge badge-danger badge-pill`}>Compaign</span>
                              }</td>
                              <td>{user.status == 1 ?
            ...
            <Modal isOpen={this.state.addNewUserModal} toggle={() => this.onAddUpdateUserModalClose()}>
               <ModalHeader toggle={() => this.onAddUpdateUserModalClose()}>
                  {editUser === null ?
                     <IntlMessages id="sidebar.addnewcategory" /> : <IntlMessages id="sidebar.updatecategory" />
                  }
               </ModalHeader>
               <ModalBody>
                  {editUser === null ?
                        ...
                        <div className="clearfix d-flex">
                           <div className="media pull-left">
                              ...
                              <div className="media-body">
                                 ...
                                 <p>Type: {selectedUser.type == 1 ?
                                 <span className={`badge badge-success badge-pill`}>Blog</span> : <span className={`badge badge-danger badge-pill`}>Compaign</span>
                              }</p>
                                 {selectedUser.campaign_type !== '' && selectedUser.campaign_type !== null  &&
                                  <p>Comapign Type: {selectedUser.campaign_type == 1 ?
                                 <span className={`badge badge-success badge-pill`}>Programming</span> : <span className={`badge badge-danger badge-pill`}>Hardware</span>
                              }</p>
                                }
