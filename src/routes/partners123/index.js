...
export default class Changelog extends Component {
  ...
  constructor() {
      ...
      let self = this;
       api.post('deletechangelog',{
              ...
              const user =  users.slice(offset, offset + AppConfig.paginate);
               ...
               NotificationManager.success('Partner Deleted!');
            }, 2000);
         })
         .catch(error => {
            // error hanlding
   ...
   addNewUser() {
     ...
     const { name, auth_token } = this.state.addNewUserDetail;
      ...
      if (name !== '' ) {
         ...
         api.post('addchangelog',newUser ).then((response) => {
            ...
            data.checked = false;
             ...
             currentCountries.unshift(data);
                setTimeout(() => {
                  ...
                  NotificationManager.success('Changelog Created!');
               }, 5000);       
         })
         .catch(error => {
            // error hanlding
   ...
   onSort(column){
      ...
      const sortedData = this.state.currentCountries.sort((a, b) => {
        if (column === 'name_slug' || column === 'text_slug') {
  ...
  setArrow = (column) => {
         ...
         let newList = null;
          ...
          if (query !== "") {
            ...
            newList = currentList.filter(item => {
                return item.name_slug.toLowerCase().includes(query.toLowerCase()) 
              });
          } else {
    ...
    */
    ...
    updateUser() {
      const { editUser, auth_token } = this.state;
      ...
      let self = this;
      api.post('updatechangelog', editUser).then((response) => {
        const data =  response.data;
       
        users[indexOfUpdateUser] = data;
            setTimeout(() => {
              self.setState({ loading: false, currentCountries:users, cropResult: '', });
              NotificationManager.success('Changelog Updated!');
           }, 5000);       
         })
         .catch(error => {
            // error hanlding
     ...
     if(search){
      ...
      return (
         <div className="user-management">
             <IntlMessages id='sidebar.changelogs' defaultMessage='Chameleon | Partner Manager'>
               {(title) => (
                 <Helmet>
               ...
               <div className="table-responsive">
                  <div className="d-flex justify-content-between py-20 px-10 border-bottom">
                     ...
                     <div>{/*
                        ...
                        <a href="javascript:void(0)" onClick={() => this.opnAddNewUserModal()} color="primary" className="caret btn-sm mr-10">{<IntlMessages id="sidebar.addnewchangelog" />} <i className="zmdi zmdi-plus"></i></a>
                     </div>
                  </div>
                  <table className="table table-middle table-hover mb-0">
                     <thead>
                        <tr>
                          ...
                          <th onClick={e => this.onSort('slug')}>{<IntlMessages id="sidebar.name-slug" />} <i className={this.setArrow('slug')}></i></th>
                          <th onClick={e => this.onSort('value_en')}>{<IntlMessages id="sidebar.text-slug" />} <i className={this.setArrow('value_en')}></i></th>
                     ...
                     <tbody>
                        {filteredResult && filteredResult.map((user, key) => (
                           <tr key={key}>
                               ...
                               <td>{user.name_slug}</td>
                              <td>{user.text_slug}</td>
                              <td>{user.status == 1 ?
            ...
            <Modal isOpen={this.state.addNewUserModal} toggle={() => this.onAddUpdateUserModalClose()}>
               <ModalHeader toggle={() => this.onAddUpdateUserModalClose()}>
                  {editUser === null ? <IntlMessages id="sidebar.addNewchangelog" /> : <IntlMessages id="sidebar.updatechangelog" />
                  }
               </ModalHeader>
               <ModalBody>
                  {editUser === null ?
                        ...
                        <div className="clearfix d-flex">
                           <div className="media pull-left">
                              ...
                              <div className="media-body">
                                 <p>{<IntlMessages id="sidebar.name-slug" />}: <span className="fw-bold">{selectedUser.name_slug}</span></p>
                                 <p>{<IntlMessages id="sidebar.text-slug" />}: <span className="fw-bold">{selectedUser.text_slug}</span></p>
                                 <p>{<IntlMessages id="widgets.status" />}: <span className="fw-bold">{selectedUser.status}</span></p>
