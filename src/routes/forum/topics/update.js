...
import { timeAgo, textTruncate, checkPath, checkPaths, hubCheckPaths, pathForxml  ,
    getFileExt ,get_player_url , formatBytes,getFileType 
      , is_moderator} from "Helpers/helpers";
...
export default class TopicUpdate extends Component {
...
// for tags end
  componentDidMount() {
    ...
    this.setState(state => ({ addNewUserDetail: [...state.addNewUserDetail, this.state.topic] }));
    const  topic = this.state.topic;
    setTimeout(() => {
            console.log("topic",this.state.topic)
     }, 500);  
     
    //  get-topic-detail-by-id-for-update
    

       
     api.get('forum/get-topic-detail-by-id-for-update' ,{
        params: {
            'id':topic.id
        },
     ...
     this.setState({
        ...
        addNewUserDetail: {
            ...this.state.addNewUserDetail,
                ...
                ans_3:  topic.ans_3,
                ans_4:  topic.ans_4,
                right_ans:  topic.right_ans,
                video_url:  topic.video_url,
                website_url:  topic.website_url,
        },
        gallary:topic.gallary,
        files:topic.files
    });
    ...
    api.post('forum/get-all-tags-list-by-topic-id' ,{'topic_id':this.state.topic.id} )
        .then((response) => {
            this.setState({mytags:response.data.tags });
        }) 
   ...
   for (var key in str) {
     ...
     case 'website_url':
            if(str['topic_type_id'] == 6){
                ...
                if (!re.test(str[key])) { 
                   formIsValid = false;
                   NotificationManager.error('Website Url is not valid!');
                 }
            }
...
console.log(key,value)
   ...
   addNewUser() {
        ...
        if(validateField){
            ...
            api.post('forum/add-topic',data,{
                headers: {'User-Id':user_id()}
            },).then((response) => {
                const data =  response.data;     
                    NotificationManager.success('Topic Updated Successfully!');
                    setTimeout(() => {
...
deleteImagePermanently() {
    ...
    // let gallary = this.state.gallary;
    // let indexOfDeleteUser = gallary.indexOf(selectedUser);
 
    this.refs.deleteConfirmationDialog.close();
    this.setState({ loading: true });
    let self = this;
    api.post('forum/delete-topic-image', {
    'id': selectedUser.id,
    }).then((response) => {
       
        setTimeout(() => {
            // gallary.splice(indexOfDeleteUser, 1);
...
//===============================================================================================
   render() {
    const { loading ,gallary,files ,slim_images,categories ,forums,topic_types ,topic_type_id , addNewUserDetail , redirectToTopics,mytags, suggestions} = this.state;
     //Dropzone
    ...
    <div className="background-white container"  style={{width:"88%"}}>
        <div className="tt-wrapper-inner">
            <h1 className="tt-title-border">
                <IntlMessages id="widgets.UpdateTopic"/>
            </h1>
                ...
                <div className="form-group">
                    ...
                    <div className="tt-js-active-btn tt-wrapper-btnicon">
                        ...
                        { topic_types && topic_types.map((topic_type, key) => (
                             ...
                             <div key={topic_type.id}  className={(topic_type.id ==  addNewUserDetail.topic_type_id)?'col-4 col-lg-3 col-xl-2 topic_types active topicDiv'+topic_type.id :'col-4 col-lg-3 col-xl-2 topic_types topicDiv'+topic_type.id   } for="comment" onClick={(e) => this.changeTopicTypes(topic_type.id , 'topicDiv'+topic_type.id)}>
...
{/*============================================== new image=============================================== */}
   {/* { topic_type_id == 4 &&
        <div>
           <div className="row">
                { gallary && gallary.map((file, key) => (
                    file.file_name !== '' &&  file.file_name !== null &&  file.file_name  !== 'undefined' ?
                    <div className={"col-sm-2 topic-pic imageId"+file.id}>
                        <img src={hubCheckPaths('forum_images')+ file.file_name } alt={file.file_name} className=" m-1 " height='150' width="150" />
                        <div className="delete-topic"><a href="javascript:void(0)"   onClick={() => this.onImageDelete(file)} > <span className="material-icons"  >delete_forever</span></a></div>
                    </div>
                    : ""
                )) }
            </div>
...
{/*============================================== new image=============================================== */}
               ...
               { topic_type_id == 2 &&
                      <div className="form-group">
                      <label for="inputTopicTitle"><IntlMessages id="widgets.TopicBody" />2</label>
                      <ReactQuill
                              modules={modules}
                              formats={formats}
                              name="description"
            ...
            {/* =========================================type 3=================================================================== */}
                ...
                <div>
                    ...
                    <div key="right_ans" className="form-group">
                        <label for="inputTopicTitle"><IntlMessages id="components.SelectRightAnswer"/></label>
            ...
            { topic_type_id == 4 &&
                    ...
                    <FormGroup>
                        <Label ><IntlMessages id="sidebar.image" /></Label>
                            <Slim
                                ratio="512:512"
                                data-size="512,512"
                                initialImage={this.state.topic.file ?  hubCheckPaths('forum_images')+this.state.topic.file : null} 
                ...
                { topic_type_id != 4 &&
                    ...
                    <div className="col-md-12 comment_files" >
                     ...
                     <div className="row">
                        <div className="col-auto ml-md-auto">
                            <a href="#" onClick={(e) => this.addNewUser(e)} className="btn btn-secondary btn-width-lg"><IntlMessages id="widgets.UpdateTopic" /></a>
                        </div>
                    </div>
                    {loading &&
                  <RctSectionLoader />
...
<DeleteConfirmationDialog
               ...
               onConfirm={() => this.deleteImagePermanently()}
            />
</div>
           );
   }
